...
public class ShellSort {
	...
	public static void main(String[] args) {
		...
		intervalSort(arr);
		System.out.println("--쉘 정렬 이후---------------");
		System.out.println(Arrays.toString(arr));

	}
	//interval을 4, 2, 1 순으로 건너뛰기 하면서 삽입정렬을 수행한다=> 쉘정렬 
		...
		for(int interval=arr.length/2; interval>0; interval=interval/2) {
			...
			//interval: 4,2,1 순으로 진행된다. =>수열 
				...
				for(j=k-interval; j>=0; j-=interval) {
					if(arr[j]>tmp) {
						arr[j+interval]=arr[j];//tmp보다 큰 값을 일정간격 만큼 건너뛴 부분에 채워넣기(뒤로 밀어넣기) 
					}else break;
				}
			}//for--------------
		}//for-------------------
	...
	public void intervalSort2(int[] arr) {
		//interval의 초기값을 구하자.
		int inter;
		for(inter=1; inter<arr.length; inter=inter*3+1) {
			System.out.println("inter: "+inter);
			for(;inter>0; inter=inter/3) {
